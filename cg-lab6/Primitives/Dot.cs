...
namespace cg_lab6.Primitives
{
    ...
    class Dot
    {
        ...
        private PointF perspective { get; set; }
        private Color color { get; set; }
        private PointF worldCenter { get; set; }
        ...
        public PointF getProjection(ProjectionType projection) 
        {
            if (projection == ProjectionType.Central) 
            {
                ...
                return new PointF(Constants.WORLD_X + (float)(res.matrix[0, 0] / res.matrix[3,0]), Constants.WORLD_Y + (float)(res.matrix[1, 0] / res.matrix[3, 0]));
            }
            else 
            {
                ...
                MatrixUtils m1 = new MatrixUtils(3,3,sqrt3,0,-sqrt3,1,2,1,sqrt2,-sqrt2,sqrt2);
